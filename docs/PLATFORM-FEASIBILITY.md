...
| Generic HTML5 video | `experimental` implementation | Toolbar-triggered `activeTab` detector reads standard `HTMLMediaElement` playback context in the page and same-origin frames | Cross-origin frames, captions, blob/MSE sources, DRM, canvas tainting, and site-specific policy remain unresolved | Run representative LMS-page tests before promoting the capability |
| Blackboard Learn | `experimental` as a container | The generic detector handles accessible HTML5 video and reports probable cross-origin provider hostnames | Tenant-specific domains and policies; Kaltura, Panopto, Vimeo, YouTube, Collaborate, and native files behave differently | Use live diagnostics to choose the next provider-specific adapter; do not claim blanket Blackboard support |
